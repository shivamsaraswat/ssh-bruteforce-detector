...
# Regex for looking for timestamp and username
LOG_LINE = re.compile("(^[A-Za-z]{3}[ ][0-9]{1,2}[ ][0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2}).*for[ ]([a-zA-Z0-9]+.*)from")
...
# Receives a log line, parses and converts date format into epoch seconds.
...
# Checks if brute-force attack is in the works.
    def feed_line(self, line):
        ...
        entry = re.search(LOG_LINE, line.strip())

        if not entry:
            print(line)

        if entry:
            time_string, user = re.findall(LOG_LINE, line.strip())[0]
...
if __name__ == "__main__":
    ...
    brute_force_detection = FileReader(LOG_FILE)
